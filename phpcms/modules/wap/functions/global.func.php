...
function content_pages($num, $curr_page,$pageurls,$showremain = 1) {
	...
	if($curr_page>0) {
		...
		$multipage .= '<li><a class="a1" href="'.$pageurls[$perpage][1].'">'.L('previous').'</a></li>';
	}

	if($curr_page<$pages) {
		if($curr_page<$pages-5 && $more) {
			$multipage .= '<li><a class="a1" href="'.$pageurls[$curr_page+1][1].'">'.L('next').'</a></li>';
		} else {
			$multipage .= '<li><a class="a1" href="'.$pageurls[$curr_page+1][1].'">'.L('next').'</a></li>';
		}
	} elseif($curr_page==$pages) {
		$multipage .= '<li><a class="a1" href="'.$pageurls[$curr_page][1].'">'.L('next').'</a></li>';
	}
